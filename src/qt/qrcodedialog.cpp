...
{
    QString ret = QString("xna:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non XNA unit input in lnReqAmount, we generate the URI with XNA as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::XNA, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
