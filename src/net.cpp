...
bool GetMyExternalIP(CNetAddr& ipRet)
{
    ...
    for (int nHost = 1; nHost <= 2; nHost++)
    {
        ...
        {
            addrConnect = CService("67.211.45.66", 80); // ip.deoxyribose.net

            if (nLookup == 1)
            {
                CService addrIP("ip.deoxyribose.net", 80, true);
                if (addrIP.IsValid())
                    addrConnect = addrIP;
            }
...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "DeOxyRibose " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
void ThreadDNSAddressSeed(void* parg)
{
    ...
    RenameThread("deoxyribose-dnsseed");
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. DeOxyRibose is probably already running."), addrBind.ToString().c_str());
        else
